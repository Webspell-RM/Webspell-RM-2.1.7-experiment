...
class SecurityHelper {
    ...
    // Überprüfen des Logins (Passwort und E-Mail)
    ...
    public static function trackFailedLogin(?int $userID, string $email, string $ip): void
    {
        ...
        $stmt = $_database->prepare("
            INSERT INTO failed_login_attempts (userID, ip, attempt_time, status, reason)
            VALUES (?, ?, NOW(), ?, ?)
        ");
        $stmt->bind_param("isss", $userID, $ip, $status, $reason);
        $stmt->execute();
        $stmt->close();
    }
