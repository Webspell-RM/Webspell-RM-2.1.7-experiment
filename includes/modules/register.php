...
// by ZENITH-Developments.de # Datenbankeinträge lesen (?)
$_admin_minpasslen = "6";  // Mindestlänge des Passworts
$_admin_maxpasslen = "";   // Maximale Passwortlänge (leer bedeutet keine Obergrenze)
$_admin_musthavelow = true;  // Muss Kleinbuchstaben enthalten?
$_admin_musthaveupp = true;  // Muss Großbuchstaben enthalten?
$_admin_musthavenum = true;  // Muss Zahlen enthalten?
$_admin_musthavespec = true; // Muss Sonderzeichen enthalten?

// Funktion zur Überprüfung der Komplexität des Passworts
function pass_complex($pwd, $_admin_minpasslen, $_admin_maxpasslen, $_admin_musthavelow, $_admin_musthaveupp, $_admin_musthavenum, $_admin_musthavespec) {
    // Überprüfen, ob Kleinbuchstaben erforderlich sind
    if ($_admin_musthavelow == true) { 
        $_pwd_low = "(?=\S*[a-z])"; 
    } else { 
        $_pwd_low = ""; 
    }

    // Überprüfen, ob Großbuchstaben erforderlich sind
    if ($_admin_musthaveupp == true) { 
        $_pwd_upp = "(?=\S*[A-Z])"; 
    } else { 
        $_pwd_upp = ""; 
    }

    // Überprüfen, ob Zahlen erforderlich sind
    if ($_admin_musthavenum == true) { 
        $_pwd_num = "(?=\S*[\d])"; 
    } else { 
        $_pwd_num = ""; 
    }

    // Überprüfen, ob Sonderzeichen erforderlich sind
    if ($_admin_musthavespec == true) { 
        $_pwd_spec = "(?=\S*[\W])"; 
    } else { 
        $_pwd_spec = ""; 
    }

    // Das Passwort mit den festgelegten Regeln prüfen
    if (!preg_match_all('$\S*(?=\S{'.$_admin_minpasslen.','.$_admin_maxpasslen.'})'.$_pwd_low.$_pwd_upp.$_pwd_num.$_pwd_spec.'\S*$', $pwd)) {
        return false;  // Wenn das Passwort die Regeln nicht erfüllt, zurückgeben: false
    }
    return true;  // Wenn alle Regeln erfüllt sind, zurückgeben: true
}

// Versuchen, die reCAPTCHA-Einstellungen aus der Datenbank zu holen
try {
    // SQL-Abfrage zum Abrufen der reCAPTCHA-Einstellungen
    $get = mysqli_fetch_assoc(safe_query("SELECT * FROM `".PREFIX."settings_recaptcha`"));
    // Die Web- und Secret-Keys für reCAPTCHA aus der Datenbank holen
    $webkey = $get['webkey'];
    $seckey = $get['seckey'];

    // Überprüfen, ob reCAPTCHA aktiviert ist
    if ($get['activated'] == "1") { 
        $recaptcha = 1;  // reCAPTCHA aktiviert
    } else { 
        $recaptcha = 0;  // reCAPTCHA nicht aktiviert
    }
} Catch (EXCEPTION $e) {
    // Falls ein Fehler auftritt (z. B. keine Verbindung zur DB), wird reCAPTCHA deaktiviert
    $recaptcha = 0;
}

// Sprachmodul für die Registrierung laden
$_language->readModule('register');

// Template-Daten für den Kopfbereich der Registrierungsseite vorbereiten
$data_array = array();
$data_array['$title'] = $_language->module['title'];  // Den Titel aus dem Sprachmodul holen
$template = $tpl->loadTemplate("register", "head", $data_array);  // Template für den Kopfbereich laden
echo $template;  // Das Template ausgeben
