...
AccessControl::checkAdminAccess('ac_update');
...
function curl_json2array($url) {
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE); // SSL-Prüfung deaktiviert
...
function check_server($url) {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_NOBODY, true); // Nur Header anfordern
    ...
    return ($http_code >= 200 && $http_code < 400); // Erfolgreiche HTTP-Statuscodes prüfen
}

$primary_url = $updateserverurl.'/base/vupdate.php';
$backup_url = $dangerupdateserverurl.'/base/vupdate.php';

if (check_server($primary_url)) {
    $update = file_get_contents($primary_url);
    $server_status = '(Alpha Server)';
} elseif (check_server($backup_url)) {
    $update = file_get_contents($backup_url);
    $server_status = '(Beta Server)';
} else {
    $update = null;
    $server_status = '(Kein Server verfügbar)';
}

// Falls $update leer ist, entsprechende Meldung ausgeben
if ($update === null) {
    echo "Fehler: Kein Update-Server verfügbar.";
}


// Beispiel-Aufruf der Funktion:
#$updateserverurl = "https://example.com"; // Haupt-Update-Server
#$dangerupdateserverurl = "https://backup.com"; // Backup-Update-Server

#fetch_update($updateserverurl, $dangerupdateserverurl);
